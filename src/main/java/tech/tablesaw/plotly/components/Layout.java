...
public class Layout {
    ...
    protected Map<String, Object> getContext() {
        ...
        if (!titleFont.equals(DEFAULT_TITLE_FONT)) {
        ...
        if (hoverDistance != DEFAULT_HOVER_DISTANCE) {
            context.put("hoverdistance", hoverDistance);
        }
        if (!hoverMode.equals(DEFAULT_HOVER_MODE)) {
        ...
        if (!dragMode.equals(DEFAULT_DRAG_MODE)) {
            context.put("dragmode", dragMode);
        }
        if (showLegend != null) {
            context.put("showlegend", showLegend);
        }
        if (!plotBgColor.equals(DEFAULT_PLOT_BG_COLOR)) {
            context.put("plotbgcolor", plotBgColor);
        }
        if (!paperBgColor.equals(DEFAULT_PAPER_BG_COLOR)) {
            context.put("paperbgcolor", paperBgColor);
        }
        if (!barMode.equals(DEFAULT_BAR_MODE)) {
